...
# this is the text veraion of the game
bugs = 0
import time
import os
os.system("color F0")
...
# add level parameter
    ...
    while True:
        ...
        elif player.relic != "e-sword" and player.used_e_sword_before != 1:
            left = 1 - player.bought["sword"]
            text = f"{i} Sword (+1 atk) - 2 ({left} left)"
            if left == 0:
                text = GREY + text + RESET
            print(text)
            options[str(i)] = "sword"
            i += 1
        else:
            left = 5 - player.bought["battery"]
            text = f"{i} Battery for Countering - 2 ({left} left)"
            if left == 0:
                text = GREY + text + RESET
            print(text)
            options[str(i)] = "battery"
            i += 1

        # Shield
        left = 1 - player.bought["shield"]
        text = f"{i} Shield (+1 def) - 2 ({left} left)"
        if left == 0:
            text = GREY + text + RESET
        print(text)
        options[str(i)] = "shield"
        i += 1

        # Boots (blocked by Cursed Socks)
        if player.artefact == "socks":
            text = f"{i} Boots (+1 spd) - cannot buy due to Cursed Socks-some bad curse will occur if you try putting that sock into a sealed space, so this game might crash"
            print(GREY + text + RESET)
        else:
            left = 1 - player.bought["boots"]
            text = f"{i} Boots (+1 spd) - 2 ({left} left)"
            if left == 0:
                text = GREY + text + RESET
            print(text)
            options[str(i)] = "boots"
        ...
        # Chicken
        if player.relic == "sk":
            left = 3 - player.bought["chicken"]
            text = f"{i} Chicken (+7 hp if missing >=5) - 2 ({left} left)"
        elif player.relic == "bk":
            left = 1 - player.bought["chicken"]
            text = f"{i} Chicken (+7 hp if missing >=5) - 2 ({left} left)"
        elif player.relic == "gk":
            left = 9999999 - player.bought["chicken"]
            text = f"{i} Chicken (heal max) - 2 (∞ left)"
        elif player.relic == "dk":
            left = 9999999 - player.bought["chicken"]
            text = f"{i} Chicken (heal max) - 2 (∞ left)"
        else:
            left = 1 - player.bought["chicken"]
            text = f"{i} Chicken (+5 hp if missing >=5) - 2 ({left} left)"

        if left == 0:
            text = GREY + text + RESET
        print(text)
        options[str(i)] = "chicken"
        i += 1


        # Books
        for key, name in [("atk","Book of Attack (x2 atk)"),
                          ("def","Book of Defence (x2 def)"),
                          ("spd","Book of Speed (x2 spd)"),
                          ("hp","Book of Max HP (x2 max hp)"),
                          ("heal","Book of Healing (hp → max hp)")]:
            left = 3 - player.books[key]
            text = f"{i} {name} - 3 ({left} left)"
            if left == 0:
                text = GREY + text + RESET
            print(text)
            options[str(i)] = f"book_{key}"
            i += 1

        if player.artefact == "devotion":
            left = 1 - player.bought_elixir_this_turn
            text = f"{i} Elixir (*onster) - max hp-{player.bought['elixir']+1}, spd-1, extra spd+6, extra spd -2 after fight for 3 turns - 1 ({left} left)"
            if left == 0:
                text = GREY + text + RESET
            print(text)
            options[str(i)] = "elixir"
            i += 1

        # Forbidden Ink
        if player.relic == "forbidden":
            left = 10 - player.forbidden_ink_total  # max 10 purchases
            text = f"{i} Forbidden Ink (insta-kill next monster when entering next fight) - 7 ({left} left)"
            if left == 0:
                text = GREY + text + RESET
            print(text)
            options[str(i)] = "ink"
        ...
        # 0 → start fight, 505 → show descriptions
        print("0 Start fight")
       # print("505 Show item descriptions")
        #print("505 Show current stats (after entering, might need to scroll a bit up to see)")

        # 235 for radio active stuff
        if player.artefact == "ar":
            print(f"235 Test water sample (max hp-5, money+{((level-1)//5)**3})")

        # 777 → activate cursed socks (after listing all items)
        if player.artefact == "socks":
            print("777 Activate cursed socks")


# Carrier of Legend: Switch relic option
        if player.artefact == "carrier":
            print("888 Switch to next relic (currently active: {})".format(player.relic))
            options["888"] = "switch_relic"

        if sf == True:
            print ("999 turn off printing normal attacks in fight")
        if sf == False:
            print ("999 turn on printing normal attacks in fight")
        ...
        if choice == "999":
            ...
            if sf == False:
                ...
                continue
#        if choice == "505":
            #print_shop_descriptions(player)
            #continue
        #if choice == "505":
            #display_player_and_next_monster(player, level)
            #continue
...
# Sock (2.1, no spd charge, tested for balance):
        if choice == "777" and player.artefact == "socks":
            if player.socks_sealed:
                print("Cursed Socks are already activated.")
                continue

            if player.hp > 3:
                print("Cursed Socks cannot activate unless HP is 3 or less.")
                continue
# discarded speed punishment mechaniqe, but kept here with # just in case of future uses in other stuff, I'm so smart😎

            # try:
                # print(f"Your current SPD is {player.spd}.")
                # spd_loss = int(input("How much SPD do you want to sacrifice? "))

                # if spd_loss <= 0:
                    # print("You must sacrifice >0 SPD.")
                    # continue
                # if spd_loss > player.spd:
                    # print("You cannot sacrifice more SPD than you currently have.")
                    # continue
            # except ValueError:
            ...
            atk_gain = hp_lost # * spd_loss

            # player.spd -= spd_loss
            player.atk += atk_gain
            player.max_hp = player.hp
            player.socks_sealed = True

            print(
                f"Cursed Socks activated!\n"
                f"- ATK increased by {atk_gain}\n"
                f"- Max HP sealed at {player.max_hp}"
            )

            continue  # return to shop

        if choice == "235" and player.artefact == "ar":
            if player.not_ar == True:
                player.max_hp -= 5
                player.hp = min(player.hp, player.max_hp)
                arb = ((level-1)//5)**3
                if player.yd4 == 4:
                ...
                player.money += arb
                player.not_ar = False
                print(f"You collected some Blight (glowing water) in the dungeon, despite knowing it is radioactive(crused), you still decided to betray your DNA(soul) and melded with(drank) it.\nReading your treatise(report), the demons(people at the summit of glorious achievement) decided to bestow(reward) you with {arb} money, given the deepness the sample is from.\nYou feel your body is already regretting this decision, max hp -5!")

            else:
                print("You can't risk in taking another sample of voidous venom(contaminated water)")
            continue
        if choice == "0": #esss
            # Coinpot effect activates immediately after leaving shop
            if player.artefact == "coinpot" and player.money >= 3:
                bonus = (player.money // 3)*2
                player.coinpot_bonus += bonus
            if player.artefact == "vitality":
                player.ppa = True

            bought_elixir_this_turn = 0 #reset count per turn, when player exit shop (presses 0)
            player.c = c
            player.cc = cc
            player.ccc = ccc
            player.bought_elixir_this_turn = bought_elixir_this_turn

            if get_atk(player) >= level+level+4:
                player.osp = True
            else:
                player.osp = False
            atk = get_atk(player)
            spd = get_spd(player)
            df = get_df(player)
            print(f"\nYou will deal {atk}-{level}={max(atk-level,1)}(at least 1) dmg,")
            print(f"\nMonster will deal {level}-{df}={max(level-df,1)}(at least 1) dmg")
            print(f"Your spd is {spd} compared to the monster's {level},")
            if spd >= level:
                print("so you will go first!")
            else:
                print("so the monster will go first!")
        ...
        # Track first equipment purchase for Discount Ticket
        first_equipment = False
        if item in ["sword","shield","boots"] and player.money <= 1:
            print(f"Cannot buy {item.capitalize()} — not enough money.")
        if item in ["sword","shield","boots"] and player.bought[item] >= 1:
            print(f"Cannot buy {item.capitalize()} — purchase limit reached.")
            continue



        if item in ["sword","shield","boots"] and player.money >= 2 and player.bought[item] < 1:
            if item == "sword":
                player.atk += 1; player.money -= 2; player.bought["sword"] += 1
            elif item == "shield":
                player.df += 1; player.money -= 2; player.bought["shield"] += 1
            elif item == "boots":
                if player.artefact == "socks":
                    print("Cannot buy Boots — no Cursed Socks inside sealed spaces.")
                    continue
                if player.relic == "meowton":
                    player.extra_atk -= player.spd
                player.spd += 1; player.money -= 2; player.bought["boots"] += 1
                if player.relic == "meowton":
                    player.extra_atk += player.spd

        elif item == "yds":
            player.extra_atk -= player.ydatk
            while True:
                print(f"Money: {player.money}")
                # --- UPGRADE 1 ---
                print(f"RAY-24    Drone atk:{player.ydatk}    Upgrade slot {player.ydus}/6")
                print(f"\n1 Hardpoint munition upgrade-stage {player.yd1}/4")
                
                if player.yd1 == 1:
                    print(f"Stage 1-drone atk +1, cost: 1")
                else:
                    print(f"{GREY}Stage 1-drone atk +1, cost: 1{RESET}")
                    
                if player.yd1 == 2:
                    print(f"Stage 2-drone atk +2, cost: 2")
                else:
                    print(f"{GREY}Stage 2-drone atk +2, cost: 2{RESET}")
                    
                if player.yd1 == 3:
                    print(f"Stage 3-drone atk +2, cost: 3")
                else:
                    print(f"{GREY}Stage 3-drone atk +2, cost: 3{RESET}")
                    
                if player.yd1 == 4:
                    print(f"Stage 4-drone atk +8, cost: 4")
                else:
                    print(f"{GREY}Stage 4-drone atk +8, cost: 4{RESET}")

                # --- UPGRADE 2 ---
                print(f"\n2 Alpha strike-stage {player.yd2}/4")
                
                if player.yd2 == 1:
                    print(f"Stage 1-at start of fight fire Tomahawk Cruise Missile, attacking monster with drone atk({player.ydatk}), cost: 2")
                else:
                    print(f"{GREY}Stage 1-at start of fight fire Tomahawk Cruise Missile, attacking monster with drone atk({player.ydatk}), cost: 2{RESET}")
                    
                if player.yd2 == 2:
                    print(f"Stage 2-the Tomahawk Cruise Missile has double atk, cost: 2")
                else:
                    print(f"{GREY}Stage 2-the Tomahawk Cruise Missile has double atk, cost: 2{RESET}")
                    
                if player.yd2 == 3:
                    print(f"Stage 3-Tomahawk Cruise Missile penetrate defence=its attack({player.ydatk*2}), cost: 2")
                else:
                    print(f"{GREY}Stage 3-Tomahawk Cruise Missile penetrate defence=its attack({player.ydatk*2}), cost: 2{RESET}")
                    
                if player.yd2 == 4:
                    print(f"Stage 4-fires 2 more Tomahawk Cruise Missiles, Tomahawk Cruise Missiles decrease monster defence=their dmg*2, cost: 4")
                else:
                    print(f"{GREY}Stage 4-fires 2 more Tomahawk Cruise Missiles, Tomahawk Cruise Missiles decrease monster defence=their dmg*2, cost: 4{RESET}")

                # --- UPGRADE 3 ---
                print(f"\n3 FARP Protocol-stage {player.yd3}/4")
                
                if player.yd3 == 1:
                    print(f"Stage 1-when you defeat a monster heal 5, cost: 2")
                else:
                    print(f"{GREY}Stage 1-when you defeat a monster heal 5, cost: 2{RESET}")
                    
                if player.yd3 == 2:
                    print(f"Stage 2-heal max, when you defeat a monster heal 10, cost: 2")
                else:
                    print(f"{GREY}Stage 2-heal max, when you defeat a monster heal 10, cost: 2{RESET}")
                    
                if player.yd3 == 3:
                    print(f"Stage 3-heal max, when you defeat a monster heal 25, cost: 2")
                else:
                    print(f"{GREY}Stage 3-heal max, when you defeat a monster heal 25, cost: 2{RESET}")
                    
                if player.yd3 == 4:
                    print(f"Stage 4-heal max, healings(from all source) doubles, cost: 3")
                else:
                    print(f"{GREY}Stage 4-heal max, healings(from all source) doubles, cost: 3{RESET}")

                # --- UPGRADE 4 ---
                print(f"\n4 Materiel Recovery-stage {player.yd4}/4")
                
                if player.yd4 == 1:
                    print(f"Stage 1-when you defeat a monster +money = drone atk, cost: 2")
                else:
                    print(f"{GREY}Stage 1-when you defeat a monster +money = drone atk, cost: 2{RESET}")
                    
                if player.yd4 == 2:
                    print(f"Stage 2-when you defeat a monster +6 money, upgrade all stage 1 upgrades into stage 2(doesn't take up one of the 6 slots), cost: 2")
                else:
                    print(f"{GREY}Stage 2-when you defeat a monster +6 money, upgrade all stage 1 upgrades into stage 2(doesn't take up one of the 6 slots), cost: 2{RESET}")
                    
                if player.yd4 == 3:
                    print(f"Stage 3-when you defeat a monster +12 money, make all upgrades at least stage 1(does not take up slot), cost: 3")
                else:
                    print(f"{GREY}Stage 3-when you defeat a monster +12 money, make all upgrades at least stage 1(does not take up slot), cost: 3{RESET}")
                    
                if player.yd4 == 4:
                    print(f"Stage 4-double income (from all sources), upgrade all stage 3 into stage 4(doesn't take up slot), cost: 4")
                else:
                    print(f"{GREY}Stage 4-double income (from all sources), upgrade all stage 3 into stage 4(doesn't take up slot), cost: 4{RESET}")
                ...
                if ydo == 1:
                    if player.ydus >= 6:
                        print("Upgrade failed-slot is full.")
                        continue
                    else:
                        if player.yd1 == 0:
                            if player.money < 1:
                            ...
                            player.ydus += 1
                            player.ydatk += 1
                            print("upgrade purchased, drone atk + 1")
                            continue
                        if player.yd1 == 1:
                            if player.money < 2:
                                print("Upgrade failed-not enough money")
                            ...
                            player.ydus += 1
                            player.ydatk += 2
                            print("upgrade purchased, drone atk + 2")
                            continue
                        if player.yd1 == 2:
                            if player.money < 3:
                                print("Upgrade failed-not enough money")
                        ...
                        if player.yd1 == 3:
                            ...
                            player.ydus += 1
                            player.ydatk += 8
                            print("upgrade purchased, drone atk + 8")
                            continue
                        if player.yd1 >= 4:
                                print("This upgrade is already at maximum level.")
                                continue
                ...
                if ydo == 2:
                    if player.ydus >= 6:
                        ...
                        if player.yd2 == 0:
                            ...
                            player.yd2 += 1
                            player.ydus += 1
                            print("Upgrade purchased, drone will attack at start of fight!")
                            continue
                        if player.yd2 == 1:
                            if player.money < 2:
                                print("Upgrade failed-not enough money")
                            ...
                            player.yd2 += 1
                            player.ydus += 1
                            print("upgrade purchased, [that atk] atk*2")
                            continue
                        if player.yd2 == 2:
                            if player.money < 2:
                                print("Upgrade failed-not enough money")
                            ...
                            player.yd2 += 1
                            player.ydus += 1
                            print("Upgrade purchased, [that atk] will now penetrate defence!")
                            continue
                        if player.yd2 == 3:
                            if player.money < 4:
                                print("Upgrade failed-not enough money")
                            ...
                            player.yd2 += 1
                            player.ydus += 1
                            print("Upgrade purchased, drone will now fire 2 more [that] and [that] reduces monster def!")
                            continue
                        if player.yd2 >= 4:
                                print("This upgrade is already at maximum level.")
                                continue
                ...
                if ydo == 3:
                    if player.ydus >= 6:
                        ...
                        if player.yd3 == 0:
                            ...
                            player.yd3 += 1
                            player.ydus += 1
                            print("upgrade purchased, drone will now heal you after fight!")
                            continue
                        if player.yd3 == 1:
                            if player.money < 2:
                                print("Upgrade failed-not enough money")
                            ...
                            player.ydus += 1
                            print("upgrade purchased, drone healed you max will now heal more after fight!")
                            continue
                        if player.yd3 == 2:
                            if player.money < 2:
                                print("Upgrade failed-not enough money")
                        ...
                        if player.yd3 == 3:
                            ...
                            player.ydus += 1
                            player.hp = player.max_hp
                            print("upgrade purchased, drone will now double your heal!")
                            continue
                        if player.yd3 >= 4:
                                print("This upgrade is already at maximum level.")
                                continue
                ...
                if ydo == 4:
                    if player.ydus >= 6:
                        ...
                        if player.yd4 == 0:
                            ...
                            player.yd4 += 1
                            player.ydus += 1
                            print("upgrade purchased, drone will give extra money after fight!")
                            continue
                        if player.yd4 == 1:
                            if player.money < 2:
                                print("Upgrade failed-not enough money")
                            ...
                            player.yd4 += 1
                            player.ydus += 1
                            print("upgrade purchased, drone is developed and will give extra money after fight!")
                            continue
                        if player.yd4 == 2:
                            if player.money < 3:
                                print("Upgrade failed-not enough money")
                        ...
                        if player.yd4 == 3:
                            ...
                            print("upgrade purchased, drone is enhanced and will double your income!")
                            continue
                        if player.yd4 >= 4:
                                print("This upgrade is already at maximum level.")
                                continue
...
# mark as first equipment purchased

        # Discount Ticket effect
        if first_equipment and player.artefact == "discount":
            for bonus in ["sword","shield","boots"]:
                if bonus == item:
                    continue
                if bonus == "boots" and player.artefact == "socks":
                    continue
                print(f"Discount Ticket gives free {bonus.capitalize()}!")
                if bonus == "sword" and player.bought["sword"] < 1:
                    player.atk += 1; player.bought["sword"] += 1
                elif bonus == "shield" and player.bought["shield"] < 1:
                    player.df += 1; player.bought["shield"] += 1
                elif bonus == "boots" and player.bought["boots"] < 1:
                    if player.relic == "meowton":
                        player.extra_atk -= player.spd
                    player.spd += 1; player.bought["boots"] += 1
                    if player.relic == "meowton":
                        player.extra_atk += player.spd

        elif item == "chicken":
            if player.relic == "bk":
                if player.bought["chicken"] >= 1:
                    print("Cannot buy Chicken — purchase limit reached.")
                elif player.money >= 2 and player.max_hp - player.hp >= 5:
                    player.hp += 7; player.money -= 2; player.bought["chicken"] += 1
                    ...
                    if player.money < 2:
                        ...
                        print("Cannot buy Chicken — your hp is too high and would waste food if you bought chicken!")

            elif player.relic == "sk":
                if player.bought["chicken"] >= 3:
                    print("Cannot buy Chicken — purchase limit reached.")
                elif player.money >= 2 and player.max_hp - player.hp >= 5:
                    player.hp += 7; player.money -= 2; player.bought["chicken"] += 1
                    if player.hp >= player.max_hp:
                ...
                if player.money >= 2 :
                    player.hp = player.max_hp; player.money -= 2; player.bought["chicken"] += 1
                    #if player.hp >= player.max_hp:
                        #player.hp = player.max_hp
                else:
                    print("Cannot buy Chicken — not enough money")

            elif player.relic == "dk":
                player.hp = player.max_hp; player.money -= 2; player.bought["chicken"] += 1
                if player.money < 0:
                    ac["negative_money"] = True #for achievement later
            #if player.hp >= player.max_hp:
          ...
          #player.hp = player.max_hp
                ...
                if player.bought["chicken"] >= 1:
                    ...
                    player.hp += 5; player.money -= 2; player.bought["chicken"] += 1
                    if player.yd3 == 4:
                        player.hp += 5
                    print("Chicken purchase successful")
                    if player.hp >= player.max_hp:
                        player.hp = player.max_hp
                else:
                    if player.money < 2:
            ...
            if player.books[key] >= 3:
                print("Cannot buy Book — purchase limit reached.")
                ...
                if key == "heal":
                    ...
                    if player.socks_sealed:
                        print("max hp sealed by socks, don't ask how or why")
                        player.money += 3
                    else:
                        player.pending_hp_multiplier *= 2
                        print("Book of Max HP purchased! Max HP will double after your next fight.")
                    ...
                    setattr(
                        player,
                        key if key != "def" else "df",
                        getattr(player, key if key != "def" else "df") * 2
                    )
            ...
            if player.relic == 'e-sword':
                if player.bought["battery"] !=5:
                    if player.money >= 2:
                        player.counters += 1; player.money -= 2; player.bought["battery"] += 1
                    else:
                        print("Cannot buy Battery — not enough money .")
                else:
                    print("Cannot buy Battery — purchase limit reached.")
            else:
                print("Your current sword is wooden, nowhere to put in battery!")


        elif item == "ink":
            if player.forbidden_ink_total >= 10:
                print("You can't write another formula down again, it hurt too much when you got laughed at last time!")
            elif player.forbidden_ink >= 1:
                print("You already have a Forbidden Ink in your pocket, don't be greedy, ink spills easily!")
            elif player.money >= 7:
                player.forbidden_ink = 1
                player.forbidden_ink_total += 1
                player.money -= 7
                print(f"Forbidden Ink purchased! ({player.forbidden_ink_total}/10). Enter the Battle and instant kill the monster!")
            else:
                print("Not enough money for Forbidden Ink.")



        elif item == "switch_relic":
            player.switch += 1
            if player.switch >= 3:
                print("Cannot switch: you are already on your last relic!")
                continue
            if player.relic == "meowton":
                player.extra_atk -= player.spd
            ...
            player.df = 1
            player.spd = 1
            print("Your power is drained from activating multiple relics, stats go to 1!")
            player.active_relic_index = (player.active_relic_index + 1) % len(player.relics)
            player.relic = player.relics[player.active_relic_index]
            print(f"Active relic switched to: {relic_names[player.relic]}")
            ...
            if player.relic == "meowton":
                ...
                player.df -= 1
            # Handle automatic counter E-Sword swap
            if player.relic == "e-sword":
                player.used_e_sword_before = 1
        # Only gain ATK if player didn't buy a sword previously
                if player.bought.get("sword", 0) == 0:
                    player.atk += 1
                    player.counters = 0  # reset counter count
                    print("You equipped the Automatic Counter E-Sword! +1 ATK!")
                else:
                    print("You swapped your wooden sword from shop into the Automatic Counter E-Sword!")


        #dev
        elif item == "elixir":
            if player.bought_elixir_this_turn >= 1: #one can per turn
                print("Cannot buy Elixir — turn purchase limit reached.")
            elif player.money >= 1:
                player.money -= 1
                ...
                if player.hp > player.max_hp:
                    player.hp = player.max_hp
                if player.hp <= 0:
                    ac["shop_death"] = True #for achievement adding later
                    print(f"\nYou chug the elixir, hope it tasted good, as it's the last one you had.\n")
                    return "dead"

                print("You feel energised, your future self doesn't. SPD-1, extra spd +6")
                if player.relic == "meowton":
                    player.extra_atk -= (player.extra_spd+player.spd)
                player.spd -= 1
                player.extra_spd += 6
                if player.relic == "meowton":
                    player.extra_atk += (player.extra_spd+player.spd)
                ...
                print("Not enough money for the Elixir.")
